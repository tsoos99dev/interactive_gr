...
export function InfoPanel() {
  ...
  // Lie bracket
  const bracketDecomp = useMemo(() => {
    if (!state.showLieBracket) return null;
    ...
    return result.bracketChart;
  }, [state.showLieBracket, state.lieBracketFieldX, state.lieBracketFieldY, px, pz, chart]);

  // Scalar value
  const fn = scalarFunctions[state.activeScalarFn];
  const scalarValue = fn ? fn.compute(px, py, pz) : null;
    ...
    <Card className="absolute bottom-4 left-4 w-64 bg-card/80 backdrop-blur-sm pointer-events-auto py-2 gap-2">
      ...
      <CardContent className="px-4 pb-3 space-y-2">
        ...
        {scalarValue !== null && (() => {
          ...
          // Decompose gradient into basis: solve ∇f = a·e1 + b·e2
          const [e1x, e1y, e1z] = point.e1;
          const [e2x, e2y, e2z] = point.e2;
          const g11 = e1x * e1x + e1y * e1y + e1z * e1z;
          const g12 = e1x * e2x + e1y * e2y + e1z * e2z;
          const g22 = e2x * e2x + e2y * e2y + e2z * e2z;
          const t1 = e1x * gx + e1y * gy + e1z * gz;
          const t2 = e2x * gx + e2y * gy + e2z * gz;
          const det = g11 * g22 - g12 * g12;
          const ga = Math.abs(det) > 1e-10 ? (g22 * t1 - g12 * t2) / det : 0;
          const gb = Math.abs(det) > 1e-10 ? (g11 * t2 - g12 * t1) / det : 0;
        ...
        {state.showLieBracket && bracketDecomp && (
          <div className="pt-1 space-y-1 border-t border-border/50">
            <div className="text-xs font-mono">
              <span className="inline-block w-2 h-2 rounded-full mr-2" style={{ backgroundColor: "#ff8800" }} />
              <span className="text-muted-foreground">[X,Y]:</span>{" "}
              {bracketDecomp[0].toFixed(2)}·∂/∂u + {bracketDecomp[1].toFixed(2)}·∂/∂v
            </div>
          </div>
        )}
